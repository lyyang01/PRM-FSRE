...
import models
#from models.proto_ablation_study import Proto
from models.proto_7_2 import Proto
#from models.proto_ori import Proto
...
def main():
    ...
    parser.add_argument('--val', default='val_wiki',
            help='val file')
    parser.add_argument('--test', default='5-1-test-relid',
            help='test file')
    ...
    parser.add_argument('--is_pubmed', action='store_true',
           help='generate the result for submitting')
    ...
    if opt.pair:
        ...
        train_data_loader = get_loader(opt.train, sentence_encoder,
                N=trainN, K=K, Q=Q, na_rate=opt.na_rate, batch_size=batch_size, is_pubmed=opt.is_pubmed)
        val_data_loader = get_loader(opt.val, sentence_encoder,
                N=N, K=K, Q=Q, na_rate=opt.na_rate, batch_size=batch_size, is_pubmed=opt.is_pubmed)
    ...
    if model_name == 'proto':
        model = Proto(sentence_encoder, dot=opt.dot, relation_encoder=relation_encoder, is_pubmed=opt.is_pubmed, cl=opt.cl)
    elif model_name == 'gnn':
    ...
    if not opt.only_test and (not opt.test_online) and (not opt.visualize):
        ...
        framework.train(model, prefix, batch_size, trainN, N, K, Q,
                ...
                learning_rate=opt.lr, use_sgd_for_bert=opt.use_sgd_for_bert, grad_iter=opt.grad_iter, cl=opt.cl)
        
        acc = framework.eval(model, batch_size, N, K, Q, opt.test_iter, na_rate=opt.na_rate, ckpt=ckpt, pair=opt.pair, cl=opt.cl)
        ...
        if ckpt is None:
            ...
            ckpt = 'none'
        framework.test(model, batch_size, N, K, Q, opt.test_iter, na_rate=opt.na_rate, ckpt=ckpt, pair=opt.pair, output_file=opt.test_output, cl=opt.cl)
        ...
        framework.visualization(model, batch_size, N, K, Q, opt.test_iter, na_rate=opt.na_rate, ckpt=ckpt, pair=opt.pair, visual=opt.visualize)
        #print("RESULT: %.2f" % (acc * 100))
