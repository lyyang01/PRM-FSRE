...
class FewRelDataset(data.Dataset):
    ...
    FewRel Dataset
    """
    def __init__(self, name, encoder, N, K, Q, na_rate, root, is_pubmed=False):
        self.root = root
    ...
    def __getitem__(self, index):
        ...
        for i, class_name in enumerate(target_classes):
            ...
            real_support_label.append(self.rel_class[class_name])
        ...
        return support_set, query_set, query_label, relation_set, real_support_label  #separate support and query -- no pair
...
def collate_fn(data):
    ...
    batch_label = []
    batch_rel_support_label = []
    support_sets, query_sets, query_labels, relation_sets, rel_support_labels = zip(*data)
    for i in range(len(support_sets)):
        ...
        batch_rel_support_label += rel_support_labels[i]
    ...
    batch_rel_support_label = torch.tensor(batch_rel_support_label)
    
    return batch_support, batch_query, batch_label, batch_relation, batch_rel_support_label

def get_loader(name, encoder, N, K, Q, batch_size, 
        num_workers=8, collate_fn=collate_fn, na_rate=0, root='./data', is_pubmed=None):
    dataset = FewRelDataset(name, encoder, N, K, Q, na_rate, root, is_pubmed=is_pubmed)
    ##TODO
    #import pdb
    #pdb.set_trace()
...
class FewRelTestDataset(data.Dataset):
    ...
    FewRel Dataset
    """
    def __init__(self, name, encoder, N, K, Q, na_rate, root, is_pubmed=None):
        self.root = root
        path = os.path.join(root, name + ".json") # json file path
    ...
    def __getitem__(self, index):
        ...
        for idx, j in enumerate(support_set_my):
            ...
            ##for pubmed
            #if rel_set[idx] in self.pid2name.keys():
            #    name, _ = self.pid2name[rel_set[idx]]
            #    rel_text, rel_text_mask = self.__getname__(name)
            #else:
            #    rel_text, rel_text_mask = self.__getname__(rel_set[idx])
            ##
...
def get_loader2(name, encoder, N, K, Q, batch_size, 
        num_workers=8, collate_fn=collate_fn22, na_rate=0, root='./data', is_pubmed=None):
    dataset = FewRelTestDataset(name, encoder, N, K, Q, na_rate, root, is_pubmed=is_pubmed)
    ##TODO
    #import pdb
    #pdb.set_trace()
